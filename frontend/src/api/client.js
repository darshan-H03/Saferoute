...
export const authApi = {
  ...
  sync: (payload, token) =>
    apiRequest("/auth/sync", { method: "POST", body: payload, token }),
...
export const datasetApi = {
  upload: (token, formData) =>
    apiRequest("/dataset/upload", {
      method: "POST",
      body: formData,
      token,
    }),
  summary: (token) => apiRequest("/dataset/summary", { token }),
  seed: (token) => apiRequest("/dataset/seed", { method: "POST", token }),
  clear: (token, restoreSeed = false) =>
    apiRequest(`/dataset/clear?restore_seed=${restoreSeed}`, {
      method: "DELETE",
      token,
    }),
  templateUrl: () => `${API_BASE}/dataset/sample-template`,
};
