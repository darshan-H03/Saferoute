...
def get_hotspots() -> list[dict]:
    try:
        ...
        db_hotspots = CrimeHotspot.query.all()
        if db_hotspots:
            return [h.to_dict() for h in db_hotspots]
    except Exception:
        pass
...
def get_crime_meta() -> dict:
    try:
        from app.models.hotspot import CrimeHotspot
        count = CrimeHotspot.query.count()
        if count > 0:
            return {
                "title": "SafeRoute Database Hotspot Dataset",
                "source": "database",
                "hotspot_count": count,
                "city": "All mapped regions",
                "disclaimer": (
                    "Active hotspots loaded from connected database. "
                    "Historical crime and safety indicators do not guarantee current safety."
                ),
            }
    except Exception:
        pass
